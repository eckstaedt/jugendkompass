...
export class AusgabePage implements OnInit {
  ...
  async downloadPdf() {
    if (this.platform.is('capacitor')) {
      ...
      loading.present();
      this.firebaseService
        .downloadFile(this.ausgabe.pdfUrl)
    ...
    if (this.fileUploader.queue && this.fileUploader.queue.length !== 0) {
      if (this.uploadItem === 'video') {
        if (this.fileUploader.queue.length === 2) {
          if (
            this.fileUploader.queue[0].file.type === 'video/mp4' &&
            this.fileUploader.queue[1].file.type !== 'video/mp4'
          ) {
      ...
      buttons: [
        {
          ...
          handler: async () => {
            ...
            this.firebaseService
              ...
              .then(async (res: any) => {
                this.firebaseService
                  .updateAusgabe(this.ausgabe.id.toString(), {
                    pdfUrl: res.url,
                    pdfPath: res.path,
                  })
                  .then(async () => {
      ...
      buttons: [
        {
          ...
          handler: async (event: any) => {
            ...
            this.firebaseService
              ...
              .then(async (resVideo: any) => {
                this.firebaseService
                  .uploadVideoFile(this.file2)
                  .then(async (resVideoImage: any) => {
